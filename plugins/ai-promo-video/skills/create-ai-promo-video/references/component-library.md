...
5. Scaffold once. The searchable catalog stays available through `search_motion_components`, `get_motion_component`, contextual `help`, and `ai-promo://motion-library`; it is not copied into every project.
6. Call `add_advanced_video_helpers` with only the selected source groups. Required source dependencies are added automatically and existing authored files are never overwritten.
7. Import only the selected primitives. Restyle, combine, and extend them in the actual scene source.
...
The minimal scaffold contains `project.tsx`, `scene.tsx`, `review.tsx`, `format-profile.json`, and `motion-plan.json`. Optional groups such as `format.tsx`, `captions.tsx`, `ambient.ts`, `procedural.tsx`, `vector-motion.ts`, and `three-effects.ts` are copied only when selected through `add_advanced_video_helpers`. They add adaptive aspect-ratio helpers, word-timed caption animation, full-shot ambient motion, seeded flow fields and velocity-continuous particle paths, font/SVG path motion, and optional Three.js finishing passes. None supplies a default scene or visual look.

`transitions.ts` contains executable, configurable rigs for every transition-category catalog entry: directional push, zoom through, shape wipe, object carry, directional blur cut, match scale, organic morph wipe, shared-element bridge, whip pan, and deterministic displacement reveal. `camera.ts` contains dedicated dolly, ambient camera, ambient parallax, orbit, focus tracking, parallax pan, perspective tilt, a velocity-preserving `continuousCameraPath`, and an explicitly segmented `cameraPath`. These are reliable mechanisms, not mandatory designs. Change their geometry, paths, timing, easing, overlap, depth, and supporting layers, compose them with custom code, or ignore them and author a completely different effect.
