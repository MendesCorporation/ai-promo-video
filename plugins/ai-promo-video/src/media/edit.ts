...
function ffmpegPath(): string {
  const value = ffmpegPathValue as unknown as string | null;
  if (!value) throw new Error('ffmpeg-static did not provide an FFmpeg binary');
  return value;
}

function resizeFilters(resize: z.infer<typeof ResizeSchema>): string[] {
  if (resize.fit === 'stretch') return [`scale=${resize.width}:${resize.height}`];
  ...
  if (resize.fit === 'cover') filters.push(`crop=${resize.width}:${resize.height}`);
  else filters.push(`pad=${resize.width}:${resize.height}:(ow-iw)/2:(oh-ih)/2:color=${resize.background}`);
  return filters;
}

function visualFilters(options: { crop?: z.infer<typeof CropSchema>; resize?: z.infer<typeof ResizeSchema>; color?: z.infer<typeof ColorSchema>; blur?: number; removeColor?: z.infer<typeof RemoveColorSchema> }): string[] {
  const filters: string[] = [];
  if (options.crop) filters.push(`crop=${options.crop.width}:${options.crop.height}:${options.crop.x}:${options.crop.y}`);
  if (options.resize) filters.push(...resizeFilters(options.resize));
  if (options.color) filters.push(`eq=brightness=${options.color.brightness}:contrast=${options.color.contrast}:saturation=${options.color.saturation}:gamma=${options.color.gamma}`);
  if (options.blur && options.blur > 0) filters.push(`gblur=sigma=${options.blur}`);
  if (options.removeColor) filters.push(`colorkey=${options.removeColor.color.replace('#', '0x')}:${options.removeColor.similarity}:${options.removeColor.blend}`, 'format=rgba');
  return filters;
}
...
export function musicEnvelopeExpression(points: Array<{ time: number; volume: number }>): string {
  if (!points.length) throw new Error('Music envelope requires at least one point');
  for (let index = 1; index < points.length; index += 1) {
    if (points[index].time <= points[index - 1].time) throw new Error('Music envelope times must be strictly increasing');
  }
...
export async function editImage(value: ImageEditOptions): Promise<{ outputPath: string; filters: string[] }> {
...
export async function editVideo(value: VideoEditOptions): Promise<{ outputPath: string; duration: number; filters: { video: string[]; audio: string[] } }> {
  ...
  if (end <= start || end > input.duration + 0.05) throw new Error(`Invalid edit range ${start}–${end}s for ${input.duration}s input`);
  const duration = (end - start) / options.speed;
  const videoFilters = visualFilters(options);
  if (options.speed !== 1) videoFilters.push(`setpts=PTS/${options.speed}`);
  if (options.fadeIn > 0) videoFilters.push(`fade=t=in:st=0:d=${Math.min(options.fadeIn, duration)}`);
  if (options.fadeOut > 0) videoFilters.push(`fade=t=out:st=${Math.max(0, duration - options.fadeOut)}:d=${Math.min(options.fadeOut, duration)}`);

  const audioFilters: string[] = [];
  if (options.speed !== 1) audioFilters.push(...atempoFilters(options.speed));
  if (options.volume !== 1) audioFilters.push(`volume=${options.volume}`);
  if (options.fadeIn > 0) audioFilters.push(`afade=t=in:st=0:d=${Math.min(options.fadeIn, duration)}`);
  if (options.fadeOut > 0) audioFilters.push(`afade=t=out:st=${Math.max(0, duration - options.fadeOut)}:d=${Math.min(options.fadeOut, duration)}`);
...
export async function replaceVideoRange(value: ReplaceVideoRangeOptions): Promise<{ outputPath: string; duration: number; replaced: [number, number] }> {
  ...
  if (options.end > original.duration + 0.05) throw new Error(`Replacement end ${options.end}s exceeds original duration ${original.duration}s`);
...
export async function mixMusic(value: MixMusicOptions): Promise<{ outputPath: string; duration: number; envelope: Array<{ time: number; volume: number }>; preservedOriginalAudio: boolean }> {
  const options = MixMusicSchema.parse(value);
  const inputPath = resolve(options.inputVideo);
  const musicPath = resolve(options.music);
  const outputPath = resolve(options.output);
  if (inputPath === outputPath) throw new Error('Music mix output must differ from the input video');
