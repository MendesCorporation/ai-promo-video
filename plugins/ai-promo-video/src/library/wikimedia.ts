...
async function apiJson<T>(url: URL): Promise<T> {
  ...
  if (!response.ok) throw new Error(`Wikimedia Commons request failed (${response.status}): ${await response.text()}`);
...
function mediaKind(info: WikimediaImageInfo): FreeMediaKind | undefined {
  if (info.mediatype?.toUpperCase() === 'VIDEO' || info.mime?.startsWith('video/') || (info.mime === 'application/ogg' && info.duration !== undefined)) return 'video';
  if (info.mime === 'image/svg+xml') return 'svg';
  if (info.mime === 'image/gif') return 'animation';
  if (info.mime?.startsWith('image/')) return 'image';
  return undefined;
}

function normalize(page: WikimediaPage, includeShareAlike = false): FreeMediaSearchResult | undefined {
...
function searchUrl(query: string, kind: 'video' | 'asset', pageSize: number): URL {
...
async function searchWikimedia(options: {
  ...
  const payload = await apiJson<{ query?: { pages?: WikimediaPage[] } }>(searchUrl(options.query, options.media, options.pageSize ?? 12));
  return (payload.query?.pages ?? [])
    .sort((a, b) => (a.index ?? 0) - (b.index ?? 0))
    .map((page) => normalize(page, options.includeShareAlike))
    .filter((result): result is FreeMediaSearchResult => Boolean(result?.selectable))
...
async function wikimediaById(id: string): Promise<WikimediaPage> {
  const url = new URL(API);
  const parameters: Record<string, string> = {
    action: 'query',
    format: 'json',
  ...
  for (const [key, value] of Object.entries(parameters)) url.searchParams.set(key, value);
  const payload = await apiJson<{ query?: { pages?: WikimediaPage[] } }>(url);
  const page = payload.query?.pages?.[0];
  if (!page) throw new Error(`Wikimedia Commons media ${id} was not found`);
  return page;
}

export async function downloadWikimediaMedia(id: string, outputDir: string, options: { includeShareAlike?: boolean } = {}): Promise<FreeMediaSearchResult> {
  const result = normalize(await wikimediaById(id), options.includeShareAlike);
  if (!result) throw new Error(`Wikimedia Commons media ${id} has no supported downloadable file`);
