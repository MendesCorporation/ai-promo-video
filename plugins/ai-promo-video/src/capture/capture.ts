...
import { CaptureSpecSchema, type BrowserAction, type CaptureSpec } from '../types.js';
...
export interface CaptureResult {
  outputDir: string;
  captures: Array<{ id: string; path: string; url: string; title: string }>;
  recordings: Array<{ id: string; path: string; url: string; title: string; duration: number; width: number; height: number; fps: number }>;
}

async function performAction(page: Page, action: BrowserAction): Promise<void> {
  if (action.type === 'wait') {
  ...
  if (action.type === 'scroll') {
    ...
    if (action.type === 'check') {
      if (action.checked) await locator.check();
      else await locator.uncheck();
    }
  }
  if (action.delayMs) await page.waitForTimeout(action.delayMs);
}

async function settle(page: Page, waitFor?: string, delayMs = 800): Promise<void> {
...
async function normalizeRecording(input: string, output: string): Promise<void> {
...
export async function captureSite(
  spec: CaptureSpec,
  sourcePath: string,
  modes: { screenshots?: boolean; recordings?: boolean } = { screenshots: true, recordings: true },
  ...
  try {
    ...
    if (modes.screenshots !== false) {
      for (const target of parsed.targets) {
        ...
        for (const action of target.actions ?? []) await performAction(page, action);
    ...
    if (modes.recordings !== false) {
      for (const recording of parsed.recordings) {
        ...
        try {
          if (recording.preRollMs) await page.waitForTimeout(recording.preRollMs);
          for (const action of recording.actions) await performAction(page, action);
          if (recording.postRollMs) await page.waitForTimeout(recording.postRollMs);
        } finally {
    ...
    const result: CaptureResult = { outputDir, captures, recordings };
...
export async function captureFromSpecFile(path: string): Promise<CaptureResult> {
  const absolutePath = resolve(path);
  const spec = CaptureSpecSchema.parse(await readJson<unknown>(absolutePath));
  return captureSite(spec, absolutePath);
}

export async function recordFromSpecFile(path: string): Promise<CaptureResult> {
  const absolutePath = resolve(path);
  const spec = CaptureSpecSchema.parse(await readJson<unknown>(absolutePath));
  if (spec.recordings.length === 0) throw new Error('Capture spec does not define any recordings');
  return captureSite(spec, absolutePath, { screenshots: false, recordings: true });
}

export async function inspectSite(url: string, outputPath?: string): Promise<Record<string, unknown>> {
