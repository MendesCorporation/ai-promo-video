export const videoFormatIds = ['landscape', 'portrait', 'square'] as const;
export const platformTargets = ['generic', 'tiktok', 'instagram-reels', 'youtube-shorts'] as const;
...
const baseProfiles: Record<VideoFormatId, Omit<VideoFormatProfile, 'platform' | 'safeArea' | 'safeAreaPixels'>> = {
  landscape: {
    id: 'landscape',
    width: 1920,
    height: 1080,
 ...
 */
const safeAreaDefaults: Record<VideoFormatId, Record<PlatformTarget, SafeAreaInsets>> = {
  landscape: {
...
export function inferVideoFormat(width: number, height: number): VideoFormatId {
...
export function resolveVideoFormat(options: {
  ...
  for (const [side, value] of Object.entries(safeArea)) {
    if (!Number.isFinite(value) || value < 0 || value >= 0.5) throw new Error(`safeArea.${side} must be between 0 and 0.5`);
  }
  return {
    ...base,
    width,
