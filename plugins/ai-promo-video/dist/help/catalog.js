import { motionComponentCategories, motionComponentLibrary } from '../advanced/library.js';
export const helpKinds = ['tool', 'component', 'transition', 'topic'];
const parameter = (type, description, options = {}) => ({ type, description, ...options });
const tool = (id, title, summary, parameters = {}, details = {}) => ({ kind: 'tool', id, title, summary, parameters, ...details });
/**
...
const liquidGlassTextParameters = {
...
function componentParameterFallback(component) {
    return Object.fromEntries(component.parameters.map((name) => [name, parameter('author-defined value or reactive signal', `Conceptual control for ${component.title}. Inspect the source export signature and author it for the current shot; this component has no universal visual default.`)]));
}
function componentHelp(component, requestedKind = 'component') {
...
function targetLikeQuery(query) {
    const match = /^\s*(tool|component|transition|topic)\s*:\s*(.+?)\s*$/i.exec(query);
    if (!match)
        return undefined;
    return `${match[1].toLowerCase()}:${match[2]}`;
}
function resolveExact(kind, id) {
    const normalized = normalize(id);
    const searchNormalized = normalizeSearch(id);
    if (kind === 'tool')
        return toolById.get(normalized) ?? toolHelpEntries.find((entry) => normalizeSearch(entry.id) === searchNormalized);
    if (kind === 'topic')
        return topicById.get(normalized) ?? topicHelpEntries.find((entry) => normalizeSearch(entry.id) === searchNormalized);
    const component = componentById.get(normalized) ?? motionComponentLibrary.find((entry) => normalizeSearch(entry.id) === searchNormalized);
    if (!component)
        return undefined;
    if (kind === 'transition' && component.category !== 'transition')
        return undefined;
...
export function getContextualHelp(options = {}) {
    ...
    if (id) {
        ...
        if (matches.length === 1)
            return { mode: 'detail', target: `${matches[0].kind}:${matches[0].id}`, help: matches[0] };
        if (matches.length > 1)
            return { mode: 'ambiguous', requested: id, candidates: matches.map(compact) };
        return { mode: 'not-found', requested: id, suggestions: searchHelp(id, limit) };
    }
    if (options.query) {
        const promotedTarget = targetLikeQuery(options.query);
        if (promotedTarget)
            return getContextualHelp({ target: promotedTarget, limit });
...
function searchHelp(query, limit) {
