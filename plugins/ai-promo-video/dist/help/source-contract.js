...
function functionSignature(source, start) {
...
function nearestTypeDeclaration(source, before) {
...
function extractFromSource(source, file, exportName) {
    ...
    if (constMatch)
        return { exportName, sourceFile: file, callSignature: constMatch[0].trim() };
    return undefined;
}
export async function sourceApiContracts(exportNames) {
...
export async function attachSourceContracts(result) {
    if (result.mode !== 'detail' || !result.help?.sourceExports?.length)
        return result;
    return {
        ...result,
        help: {
...
//# sourceMappingURL=source-contract.js.map