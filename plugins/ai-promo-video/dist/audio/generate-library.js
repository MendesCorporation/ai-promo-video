...
function envelope(time, attack, release, length) {
    if (time < 0 || time > length)
        return 0;
    if (time < attack)
        return time / Math.max(attack, 0.001);
    return Math.min(1, (length - time) / Math.max(release, 0.001));
}
function osc(type, frequency, time) {
    const phase = fract(frequency * time);
    if (type === 'triangle')
        return 1 - 4 * Math.abs(phase - 0.5);
    return Math.sin(phase * Math.PI * 2);
}
function synthSample(time, bpm, intensity, profile) {
...
function createWav(bpm, intensity, profile) {
