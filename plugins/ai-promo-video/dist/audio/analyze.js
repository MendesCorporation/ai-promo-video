...
function numericMatch(source, expression) {
    const match = source.match(expression);
    const value = match ? Number(match[1]) : NaN;
    return Number.isFinite(value) ? value : undefined;
}
function energyCurve(stderr) {
...
function peakTimes(curve) {
    const selected = [];
    for (const point of [...curve].sort((a, b) => b.energy - a.energy || a.time - b.time)) {
        if (selected.every((time) => Math.abs(time - point.time) >= 2))
...
export async function analyzeMusic(path, reviewDir) {
