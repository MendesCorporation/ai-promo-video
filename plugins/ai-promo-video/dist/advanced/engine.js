...
import { resolveVideoFormat } from './formats.js';
...
function advancedProjectPath(projectDir, relativePath) {
    const root = resolve(projectDir);
    const target = resolve(root, relativePath);
    if (target !== root && !target.startsWith(`${root}${sep}`))
        throw new Error('File path must stay inside the advanced project directory');
    if (!advancedSourceExtensions.has(extname(target).toLowerCase()))
        throw new Error('Unsupported advanced project file type');
    return target;
}
...
export async function saveAdvancedProjectFile(projectDir, relativePath, source) {
    const target = advancedProjectPath(projectDir, relativePath);
    await mkdir(dirname(target), { recursive: true });
    await writeFile(target, source, 'utf8');
    return target;
}
export async function readAdvancedProjectFile(projectDir, relativePath) {
    const target = advancedProjectPath(projectDir, relativePath);
    return { path: target, source: await readFile(target, 'utf8') };
}
export async function listAdvancedProjectFiles(projectDir) {
    const root = resolve(projectDir);
    const files = [];
    async function visit(directory, prefix = '') {
        for (const entry of await readdir(directory, { withFileTypes: true })) {
            ...
            else if (entry.isFile() && advancedSourceExtensions.has(extname(entry.name).toLowerCase()))
                files.push(relative);
        }
    }
    await visit(root);
...
export async function patchAdvancedProjectFile(projectDir, relativePath, patches) {
    const target = advancedProjectPath(projectDir, relativePath);
    if (patches.length === 0)
    ...
    for (const patch of patches) {
        if (!patch.find)
            throw new Error('Patch find text cannot be empty');
        const occurrences = source.split(patch.find).length - 1;
        if (occurrences !== 1)
            throw new Error(`Patch text must occur exactly once in ${relativePath}; found ${occurrences}`);
...
export async function renderAdvancedProject(options) {
    ...
    if (options.rangeStart !== undefined && options.rangeEnd !== undefined && options.rangeEnd <= options.rangeStart)
        throw new Error('rangeEnd must be greater than rangeStart');
    if (!existsSync(projectFile))
        throw new Error(`Advanced project does not exist: ${projectFile}`);
    ...
    if (!sourceValidation.valid)
        throw new AdvancedSourceValidationError(sourceValidation);
    const typecheck = validateAdvancedProjectTypes(projectFile);
    if (!typecheck.valid)
        throw new AdvancedTypecheckError(typecheck);
    ...
    return new Promise((resolvePromise, rejectPromise) => {
        ...
        let diagnostics = '';
        let currentUpdate = { phase: 'starting', progress: 0, message: 'Starting isolated Revideo renderer', workers };
        let stallTimer;
        const report = (update) => {
            currentUpdate = update;
            Promise.resolve(options.onProgress?.(update)).catch(() => undefined);
        };
        const emitHeartbeat = (update) => {
            Promise.resolve(options.onProgress?.(update)).catch(() => undefined);
        };
        const terminateTree = () => {
            if (!childPid)
        ...
        child.stdout?.on('data', (chunk) => { diagnostics = `${diagnostics}${chunk.toString()}`.slice(-8_000); });
        child.stderr?.on('data', (chunk) => { diagnostics = `${diagnostics}${chunk.toString()}`.slice(-8_000); });
        ...
        child.on('exit', (code, signal) => {
            if (!settled)
                finishWithError(new Error(`Advanced renderer exited before completion (code ${code ?? 'none'}, signal ${signal ?? 'none'})`));
        });
