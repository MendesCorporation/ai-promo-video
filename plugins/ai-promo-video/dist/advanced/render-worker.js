...
import { aggregateWorkerProgress } from './render-protocol.js';
...
async function render(options) {
    ...
    const projectSettings = {
        ...
        ...(options.rangeStart !== undefined && options.rangeEnd !== undefined ? { range: [options.rangeStart, options.rangeEnd] } : {}),
    };
    const workerProgress = new Map();
    let lastSentAt = 0;
    let lastSentPercent = -1;
    let encodingAnnounced = false;
    const report = (worker, progress) => {
        ...
        const now = Date.now();
        if (percent === lastSentPercent && now - lastSentAt < 750)
            return;
        lastSentAt = now;
    ...
    const outputPath = await renderVideo({
        ...
        settings: {
            ...
            ...(ffmpegPath ? { ffmpeg: { ffmpegPath, ffprobePath: ffprobeStatic.path, ffmpegLogLevel: 'error' } } : {}),
            viteConfig: { publicDir, resolve: { alias: aliases } },
        },
    });
    const absoluteOutput = resolve(outputPath);
    if (!existsSync(absoluteOutput))
        throw new Error(`Renderer completed without creating the expected output: ${absoluteOutput}`);
...
process.on('message', (message) => {
    if (started || message?.type !== 'render')
        return;
    started = true;
