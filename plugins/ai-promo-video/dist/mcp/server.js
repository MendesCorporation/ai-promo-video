...
function errorResponse(error) {
    return {
        isError: true,
        content: [{ type: 'text', text: JSON.stringify(serializeAdvancedError(error), null, 2) }],
    };
}
...
server.registerTool('load_director_guide', {
    ...
    inputSchema: {},
}, async () => ({ content: [{ type: 'text', text: directorGuide }] }));
...
server.registerTool('search_music', {
    ...
    if (provider === 'all' || provider === 'local')
        results.push(...await searchLocalMusic({ query, directories: localDirectories, includeBundled, minDuration, maxDuration, allowUnknownLicense: allowUnknownLocalLicense }));
    if (provider === 'bundled')
        results.push(...await searchLocalMusic({ query, includeBundled: true, minDuration, maxDuration }));
    if (provider === 'all' || provider === 'openverse') {
        if (!query)
            throw new Error('Openverse search requires a query');
...
server.registerTool('read_visual_files', {
    ...
    for (const path of paths) {
        ...
        if (!mimeType)
            throw new Error(`Unsupported visual file type: ${extname(absolutePath) || '(none)'}`);
        const details = await stat(absolutePath);
        if (!details.isFile())
            throw new Error(`Visual path is not a file: ${absolutePath}`);
        if (details.size > 15 * 1024 * 1024)
            throw new Error(`Visual file exceeds the 15 MB safety limit: ${absolutePath}`);
        totalBytes += details.size;
        if (totalBytes > 40 * 1024 * 1024)
            throw new Error('Combined visual payload exceeds the 40 MB safety limit');
