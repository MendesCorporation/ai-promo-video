...
export async function extractReviewFrames(videoPath, outputDir, times) {
...
function anomalyLines(stderr) {
    return stderr
        .split('\n')
        .map((line) => line.trim())
        .filter((line) => /black_(start|end)|freeze_(start|end|duration)|silence_(start|end|duration)/.test(line));
}
export async function createVisualReviewPack(videoPath, outputDir, options = {}) {
    const ffmpegPath = ffmpegPathValue;
    if (!ffmpegPath)
        throw new Error('ffmpeg-static did not provide a binary for this platform');
    ...
    if (!(interval >= 0.25 && interval <= 10))
        throw new Error('overviewInterval must be between 0.25 and 10 seconds');
    if (!(transitionWindow >= 0.25 && transitionWindow <= 3))
        throw new Error('transitionWindow must be between 0.25 and 3 seconds');
    if (!(transitionFps >= 1 && transitionFps <= 12))
        throw new Error('transitionFps must be between 1 and 12');
    if (transitionTimes.length > 30)
        throw new Error('At most 30 transition times may be reviewed');
    if (transitionTimes.some((time) => time < 0 || time > video.duration))
        throw new Error('Transition times must stay inside the video duration');
    await mkdir(absoluteOutput, { recursive: true });
    ...
    if (video.hasAudio)
        analysisArgs.push('-af', 'silencedetect=n=-50dB:d=2');
