...
import { motionComponentLibrary, searchMotionComponents } from './advanced/library.js';
import { platformTargets, videoFormatIds, videoFormatProfiles } from './advanced/formats.js';
...
function enumFlag(value, allowed, fallback, name) {
...
export async function validateSpec(path, kind = 'video') {
...
export async function runCommandByName(name, args, flags) {
    switch (name) {
        case 'inspect': {
            const [url] = args;
            if (!url)
                throw new Error('Usage: ai-promo inspect <url> [--screenshot path]');
            return inspectSite(url, typeof flags.screenshot === 'string' ? flags.screenshot : undefined);
        }
        case 'capture': {
            if (!args[0])
                throw new Error('Usage: ai-promo capture <capture.json>');
            return captureFromSpecFile(args[0]);
        }
        case 'record': {
            if (!args[0])
                throw new Error('Usage: ai-promo record <capture.json>');
        ...
        case 'music:search': {
            ...
            if (provider === 'all' || provider === 'local')
                results.push(...await searchLocalMusic({ query, directories, includeBundled: flags['include-bundled'] === true, minDuration, maxDuration, allowUnknownLicense: flags['allow-unknown-license'] === true }));
            if (provider === 'bundled')
                results.push(...await searchLocalMusic({ query, includeBundled: true, minDuration, maxDuration }));
            if (provider === 'all' || provider === 'openverse') {
                if (!query)
                    throw new Error('Online music search requires --query');
        ...
        case 'music:download': {
            const id = args[0];
            if (!id)
                throw new Error('Usage: ai-promo music:download <openverse-id> --output-dir <dir>');
        ...
        case 'music:analyze': {
            const path = args[0];
            if (!path)
                throw new Error('Usage: ai-promo music:analyze <audio-file> [--review-dir <dir>]');
        ...
        case 'video:search': {
            ...
            return searchFreeVideos({
                ...
                provider: enumFlag(flags.provider, ['all', 'local', 'wikimedia', 'pexels'], 'all', 'provider'),
                localDirectories: typeof flags.dirs === 'string' ? flags.dirs.split(',').filter(Boolean) : [],
                orientation: typeof flags.orientation === 'string' ? enumFlag(flags.orientation, ['landscape', 'portrait', 'square'], 'landscape', 'orientation') : undefined,
        ...
        case 'video:download': {
            const id = args[0];
            if (!id)
                throw new Error('Usage: ai-promo video:download <id> --provider wikimedia|pexels --output-dir <dir>');
            const provider = enumFlag(flags.provider, ['wikimedia', 'pexels'], 'wikimedia', 'provider');
        ...
        case 'asset:search': {
            ...
            return searchFreeAssets({
                query: query || undefined,
                provider: enumFlag(flags.provider, ['all', 'local', 'openverse', 'wikimedia', 'pexels'], 'all', 'provider'),
                localDirectories: typeof flags.dirs === 'string' ? flags.dirs.split(',').filter(Boolean) : [],
                kind: enumFlag(flags.kind, ['all', 'image', 'svg', 'animation'], 'all', 'kind'),
                orientation: typeof flags.orientation === 'string' ? enumFlag(flags.orientation, ['landscape', 'portrait', 'square'], 'landscape', 'orientation') : undefined,
                minWidth: numberFlag(flags['min-width']),
                minHeight: numberFlag(flags['min-height']),
                pageSize: numberFlag(flags['page-size']),
        ...
        case 'asset:download': {
            const id = args[0];
            if (!id)
                throw new Error('Usage: ai-promo asset:download <id> --provider openverse|wikimedia|pexels --output-dir <dir>');
            const provider = enumFlag(flags.provider, ['openverse', 'wikimedia', 'pexels'], 'openverse', 'provider');
        ...
        case 'validate': {
            if (!args[0])
                throw new Error('Usage: ai-promo validate <spec.json> [--kind video|capture]');
            const kind = flags.kind === 'capture' ? 'capture' : 'video';
            return validateSpec(args[0], kind);
        }
        case 'render': {
            if (!args[0])
                throw new Error('Usage: ai-promo render <video.json> [--workers 4] [--keep-frames]');
        ...
        case 'motion:search': {
            ...
            return searchMotionComponents({
                query: query || undefined,
                categories: typeof flags.categories === 'string' ? flags.categories.split(',').filter(Boolean) : undefined,
                ...
                energy: typeof flags.energy === 'string' ? flags.energy.split(',').filter(Boolean) : undefined,
        ...
        case 'motion:get': {
            const id = args[0];
            if (!id)
                throw new Error('Usage: ai-promo motion:get <component-id>');
            const found = motionComponentLibrary.find((item) => item.id === id);
            if (!found)
                throw new Error(`Unknown motion component: ${id}`);
            return found;
        }
        case 'advanced:init': {
            const outputDir = args[0];
            if (!outputDir)
                throw new Error('Usage: ai-promo advanced:init <directory> --name <name>');
            return scaffoldAdvancedProject({
                outputDir,
                name: typeof flags.name === 'string' ? flags.name : 'Advanced AI Promo',
                format: typeof flags.format === 'string' ? enumFlag(flags.format, videoFormatIds, 'landscape', 'format') : undefined,
                platform: typeof flags.platform === 'string' ? enumFlag(flags.platform, platformTargets, 'generic', 'platform') : undefined,
        ...
        case 'caption:prepare': {
            ...
            if (!inputPath)
                throw new Error('Usage: ai-promo caption:prepare <captions.srt|vtt|json> [--output caption-timing.json]');
            return prepareCaptionTiming({ inputPath, outputPath: typeof flags.output === 'string' ? flags.output : undefined });
        }
        case 'caption:review': {
            const inputPath = args[0];
            if (!inputPath)
                throw new Error('Usage: ai-promo caption:review <caption-timing.json>');
            return reviewCaptionTiming(inputPath);
        }
        case 'advanced:render': {
            const projectFile = args[0];
            if (!projectFile)
                throw new Error('Usage: ai-promo advanced:render <project.tsx> --output <video.mp4>');
            if (typeof flags.output !== 'string')
                throw new Error('advanced:render requires --output');
        ...
        case 'advanced:patch': {
            ...
            if (!projectDir || !relativePath || !patchesFile)
                throw new Error('Usage: ai-promo advanced:patch <project-dir> <relative-file> <patches.json>');
            return patchAdvancedProjectFile(projectDir, relativePath, await readJson(resolve(patchesFile)));
        }
        case 'image:edit': {
            if (!args[0])
                throw new Error('Usage: ai-promo image:edit <edit.json>');
            return editImage(await readJson(resolve(args[0])));
        }
        case 'video:edit': {
            if (!args[0])
                throw new Error('Usage: ai-promo video:edit <edit.json>');
            return editVideo(await readJson(resolve(args[0])));
        }
        case 'video:replace-range': {
            if (!args[0])
                throw new Error('Usage: ai-promo video:replace-range <replace.json>');
            return replaceVideoRange(await readJson(resolve(args[0])));
        }
        case 'music:mix': {
            if (!args[0])
                throw new Error('Usage: ai-promo music:mix <mix.json>');
            return mixMusic(await readJson(resolve(args[0])));
        }
        case 'probe': {
            if (!args[0])
                throw new Error('Usage: ai-promo probe <video.mp4>');
            return probeVideo(args[0]);
        }
        case 'review': {
            if (!args[0])
                throw new Error('Usage: ai-promo review <video.mp4> --output-dir <dir> --times 2,8,15');
        ...
        case 'review:auto': {
            if (!args[0])
                throw new Error('Usage: ai-promo review:auto <video.mp4> --output-dir <dir> [--interval 2 --transitions 4.5,8,13]');
